/**
 * ═══════════════════════════════════════════════════════════════════════════
 *  PariScore — Tennis Live Local API (SQLite fallback)
 * ═══════════════════════════════════════════════════════════════════════════
 *
 *  Routes : /api/v1/tennis/players-local/search
 *           /api/v1/tennis/players-local/top10
 *           /api/v1/tennis/players-local/:slug
 *
 *  Intégration dans server.js — ajouter ces 2 lignes :
 *
 *    const tennisLiveApi = require('./tennis-live-api');  // ← après les autres requires
 *    // ← dans le router, AVANT la closure finale (peu importe la position, routes distinctes)
 *    if (tennisLiveApi.handleRequest(pathname, query, req.method, res)) return;
 *
 *  Le frontend tennis-live.js essaie d'abord /api/v1/tennis/players/search (BSD),
 *  puis fallback sur /api/v1/tennis/players-local/search (cette BDD locale).
 *
...
const COUNTRY_CODES = {
  'ITA': 'it', 'ESP': 'es', 'GER': 'de', 'SRB': 'rs', 'USA': 'us',
  'BLR': 'by', 'KAZ': 'kz', 'POL': 'pl', 'FRA': 'fr', 'GBR': 'gb',
  'AUS': 'au', 'RUS': 'ru', 'ARG': 'ar', 'BRA': 'br', 'CAN': 'ca',
  'CHI': 'cl', 'CRO': 'hr', 'CZE': 'cz', 'DEN': 'dk', 'FIN': 'fi',
  'GRC': 'gr', 'HUN': 'hu', 'IND': 'in', 'JPN': 'jp', 'NED': 'nl',
  'NOR': 'no', 'POR': 'pt', 'RSA': 'za', 'SUI': 'ch', 'SWE': 'se',
  'UKR': 'ua', 'URU': 'uy', 'AUT': 'at', 'BEL': 'be', 'BUL': 'bg',
  'COL': 'co', 'ECU': 'ec', 'GEO': 'ge', 'PER': 'pe', 'ROU': 'ro',
  'SVK': 'sk', 'KOR': 'kr', 'CHN': 'cn', 'TPE': 'tw', 'MON': 'mc',
  'NZL': 'nz', 'MEX': 'mx',
};
...
ON CONFLICT(slug) DO UPDATE SET
  ...
  points=excluded.points, rank=excluded.rank, peak_rank=excluded.peak_rank,
...
function init(db) {
  ...
  // Migration: add country_code column if missing (safe ALTER, ignores if exists)
  try { sqldb.exec(MIGRATE_ADD_CC); } catch (_) { /* column already exists */ }
  ...
  if (count === 0) {
    ...
    for (const p of SEED_PLAYERS) {
      stmt.run(
        p.slug, p.name, p.wikipedia_title || null, p.gender, p.country || null, p.country_code || COUNTRY_CODES[p.country] || null,
...
/**
 * Convertit une ligne DB locale vers le format BSD attendu par le frontend.
 * Format BSD : { id, name, short_name, country_code, country_name, gender, current_ranking }
 */
function toBSDFormat(row) {
  return {
    id:             row.slug,
    name:           row.name,
    short_name:     row.name ? row.name.split(' ').map(function(w){return w[0]}).join('').toUpperCase() : null,
    country_code:   row.country_code || COUNTRY_CODES[row.country] || null,
    country_name:   row.country || null,
    gender:         row.gender === 'WTA' ? 'W' : 'M',
    current_ranking: row.rank || null
  };
}

// ── Route: GET /api/v1/tennis/players-local/search?q=sinner&limit=15 ──────
// Retourne le même format que le handler BSD de server.js pour compatibilité frontend
function handleSearch(query, res) {
  ...
  if (!q || q.length < 2) return jsonResponse(res, 200, { results: [], count: 0, q: '' });

  try {
    const sqldb = getDb();
    const genderClause = gender && (gender === 'ATP' || gender === 'WTA')
      ? "AND gender = '" + gender + "'"
      : '';
    const sql = `SELECT slug, name, gender, country, country_code, rank, points, composite_score, l5, l5_win_rate
      FROM tennis_players_live
      WHERE (name LIKE '%' || ? || '%' OR country LIKE '%' || ? || '%' OR slug LIKE '%' || ? || '%' OR country_code LIKE '%' || ? || '%')
        ${genderClause}
      ORDER BY rank IS NULL, rank ASC LIMIT ?`;
    const rows = sqldb.prepare(sql).all(q, q, q, q, limit);

    // Convertir vers format BSD pour compatibilité frontend
    const results = rows.map(toBSDFormat);
    ...
    console.error('[tennis-live-api] search error:', e.message);
    return jsonResponse(res, 500, { error: e.message, results: [], count: 0 });
  }
}

// ── Route: GET /api/v1/tennis/players-local/top10?surface=hard&gender=ATP ─
function handleTop10(query, res) {
  ...
  try {
    ...
    const rows = sqldb.prepare(
      `SELECT slug, name, wikipedia_title, country, country_code, rank, points,
              ${eloCol} as surface_elo, l5, l5_win_rate, composite_score
       FROM tennis_players_live
       WHERE gender = ?
       ORDER BY rank IS NULL, rank ASC LIMIT 120`
    ).all(gender);
    ...
    const scored = rows.map(function(p) {
      ...
      return {
        ...
        officialRank: p.rank, points: p.points,
    ...
    console.error('[tennis-live-api] top10 error:', e.message);
    return jsonResponse(res, 500, { error: e.message });
  }
}

// ── Route: GET /api/v1/tennis/players-local/:slug ────────────────────────
function handleProfile(slug, res) {
  ...
  try {
    ...
    return jsonResponse(res, 200, {
      player: {
        slug: p.slug, name: p.name, wikipediaTitle: p.wikipedia_title,
        gender: p.gender, country: p.country, country_code: p.country_code || COUNTRY_CODES[p.country] || null,
        rank: p.rank, points: p.points,
        ...
        metricsSource: p.metrics_source || 'local',
        photoUrl: null,
        createdAt: p.created_at, updatedAt: p.updated_at,
      }
    });
  } catch (e) {
    console.error('[tennis-live-api] profile error:', e.message);
    return jsonResponse(res, 500, { error: e.message });
  }
}

// ── Router ────────────────────────────────────────────────────────────────
// Routes sous /api/v1/tennis/players-local/... (préfixe distinct du handler BSD)
// Retourne true si la route a été gérée, false sinon
function handleRequest(pathname, query, method, res) {
  // GET /api/v1/tennis/players-local/search
  if (pathname === '/api/v1/tennis/players-local/search' && method === 'GET') {
    handleSearch(query, res);
    return true;
  }

  // GET /api/v1/tennis/players-local/top10
  if (pathname === '/api/v1/tennis/players-local/top10' && method === 'GET') {
    handleTop10(query, res);
    return true;
  }

  // GET /api/v1/tennis/players-local/:slug
  var playersMatch = pathname.match(/^\/api\/v1\/tennis\/players-local\/([^/]+)$/);
