...
var TennisLive = (function () {
  ...
  /* ── Search ── */
  // Dual-endpoint : BSD (/api/v1/tennis/players/search) → fallback local (/api/v1/tennis/players-local/search)
  // Les deux retournent le même format BSD : { source, count, results: [{id, name, short_name, country_code, country_name, gender, current_ranking}] }
  ...
  /** Mappe un résultat BSD (ou local BSD-compatible) vers le format interne du dropdown */
  function mapSearchResult(p) {
    return {
      slug: p.id || p.short_name || String(p.name || '').toLowerCase().replace(/\s+/g, '-'),
      name: p.name || p.short_name || '—',
      rank: p.current_ranking || null,
      country: p.country_name || p.country_code || '',
      flag: p.country_code ? p.country_code.toLowerCase() : null,
      gender: p.gender || (p.name ? 'ATP' : ''),
      compositeScore: null,
  ...
  function onSchInput(v) {
    ...
    if (v.trim().length < 2) { sch.open = false; sch.results = []; renderSchDD(); return; }
    sch.loading = true;
    renderSchDD();
    sch.dt = setTimeout(function () {
      var qEnc = encodeURIComponent(v.trim());
      var primaryUrl = API_BASE + '/tennis/players/search?q=' + qEnc + '&limit=15';
      var localUrl   = API_BASE + '/tennis/players-local/search?q=' + qEnc + '&limit=15';

      // Essai 1 : endpoint BSD principal
      fetch(primaryUrl)
        .then(function (r) {
        ...
        .then(function (d) {
          var raw = d.results || [];
          // Si BSD retourne des résultats, on les utilise
          if (raw.length > 0) {
            sch.results = raw.map(mapSearchResult);
            sch.open = true;
            sch.hi = 0;
            sch.loading = false;
            renderSchDD();
          } else {
            // BSD a répondu mais 0 résultats → essai local
            fetchLocalSearch(localUrl);
          }
        })
        .catch(function () {
          // BSD indisponible (503, réseau, etc.) → essai local
          fetchLocalSearch(localUrl);
        });
    }, 250);
  }

  /** Fetch de fallback sur la BDD locale (tennis-live-api.js) */
  function fetchLocalSearch(url) {
    fetch(url)
      ...
      .then(function (d) {
        var raw = d.results || [];
        sch.results = raw.map(mapSearchResult);
        sch.open = sch.results.length > 0;
        sch.hi = 0;
        sch.loading = false;
        renderSchDD();
      })
      .catch(function (e) {
        console.warn('[TennisLive] search fallback error:', e.message);
        sch.results = [];
        sch.loading = false;
        sch.open = false;
        renderSchDD();
  ...
  function renderSchDD() {
    ...
    if (sch.loading) {
      ...
      dd.innerHTML = '<div style="padding:12px;text-align:center;color:var(--tl-text-3);font-size:11px;font-family:var(--tl-font-mono)">Recherche…</div>';
      return;
    }
    if (!sch.open || sch.results.length === 0) { dd.classList.remove('open'); return; }
    dd.classList.add('open');
    sch.results.forEach(function (p, i) {
      var flagHtml = p.flag ? '<img src="https://flagcdn.com/w20/' + p.flag + '.png" style="width:16px;height:11px;border-radius:2px;object-fit:cover;margin-right:4px" onerror="this.style.display=\'none\'" loading="lazy"/>' : '';
      var rankStr = p.rank ? '#' + p.rank : '';
      var genderBadge = p.gender ? '<span class="tl-search-badge ' + (p.gender === 'ATP' || p.gender === 'M' ? 'atp' : 'wta') + '">' + (p.gender === 'M' ? 'ATP' : p.gender) + '</span>' : '';
      var item = el('div', 'tl-search-item' + (i === sch.hi ? ' highlighted' : ''));
      item.innerHTML = '<div class="tl-search-avatar-placeholder">' + (p.name ? p.name.slice(0, 2).toUpperCase() : '??') + '</div><div style="min-width:0;flex:1"><div style="display:flex;align-items:center;gap:6px">' + flagHtml + '<span class="tl-search-name">' + p.name + '</span>' + genderBadge + '</div><div class="tl-search-meta">' + rankStr + (p.country ? ' · ' + p.country : '') + '</div></div>';
      item.addEventListener('mousedown', function (e) { e.preventDefault(); openProfile(p.slug); var inp = state.container.querySelector('.tl-search-input'); if (inp) { inp.value = ''; inp.blur(); } sch.open = false; renderSchDD(); });
      item.addEventListener('mouseenter', function () { sch.hi = i; renderSchDD(); });
      dd.appendChild(item);
    });
  }
  ...
  function renderProfile() {
    ...
    var slugEnc = encodeURIComponent(state.profileSlug);
    var primaryProfileUrl = API_BASE + '/tennis/players/' + slugEnc;
    var localProfileUrl   = API_BASE + '/tennis/players-local/' + slugEnc;

    // Essai 1 : endpoint BSD, fallback local
    fetch(primaryProfileUrl)
      .then(function (r) {
        if (r.status === 404) throw new Error('NOT_FOUND_PRIMARY');
        if (!r.ok) throw new Error('HTTP ' + r.status);
        return r.json();
      })
      .then(function (d) {
        // BSD peut retourner { player: {...} } ou directement l'objet profil
        var profileData = d.player || d;
        if (!profileData || !profileData.name) throw new Error('NOT_FOUND_PRIMARY');
        var modal = ov.children[1];
        modal.innerHTML = renderProfileHTML(profileData);
        modal.scrollTop = 0;
      })
      .catch(function (e) {
        // Si le profil BSD n'existe pas ou erreur → essai local
        if (e.message === 'NOT_FOUND_PRIMARY' || e.message.startsWith('HTTP')) {
          fetch(localProfileUrl)
            .then(function (r) {
            ...
            .then(function (d) {
              var profileData = d.player || d;
              if (!profileData || !profileData.name) throw new Error('Joueur introuvable');
              var modal = ov.children[1];
              modal.innerHTML = renderProfileHTML(profileData);
              modal.scrollTop = 0;
            })
            .catch(function (e2) {
              var modal = ov.children[1];
              modal.innerHTML = '<div style="padding:32px;text-align:center;color:#EF4444;font-size:14px;font-family:var(--tl-font-mono)">' + e2.message + '</div>';
            });
        } else {
          var modal = ov.children[1];
