        ...
        {ACTIVE_MAP_POOL.some((map) => {
          ...
          <div className="space-y-3">
            {ACTIVE_MAP_POOL.map((map) => {
              const m1 = t1.map_trends?.[map] ?? null;
              const m2 = t2.map_trends?.[map] ?? null;
              const wr1 = m1?.wr_3m ?? null;
              const wr2 = m2?.wr_3m ?? null;
              ...
              return (
                <div key={map} className="rounded-lg border border-white/5 bg-white/[0.02] p-3">
                  <div className="mb-1.5 flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-3">
                      <span className="flex items-center gap-1 text-[11px] text-orange-400">
                        {trendIcon(m1?.trend)} {wr1 != null ? `${wr1}%` : "—"}
                      </span>
                      <span className="flex items-center gap-1 text-[11px] text-blue-400">
                        {wr2 != null ? `${wr2}%` : "—"} {trendIcon(m2?.trend)}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="mt-1.5 flex items-center justify-between text-[10px] text-zinc-500">
                    <span>
                      {t1.name} {m1?.wr_6m != null ? `· 6m ${m1.wr_6m}%` : ""}
                      {m1?.map_rank_3m ? ` · #${m1.map_rank_3m}` : ""}
                    </span>
                    <span>
                      {m2?.map_rank_3m ? `#${m2.map_rank_3m} · ` : ""}
                      {m2?.wr_6m != null ? `6m ${m2.wr_6m}% · ` : ""}
                      {t2.name}
                    </span>
                  </div>
                </div>
        ...
        <section>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="rounded-lg border border-white/5 bg-white/[0.02] p-3">
              <p className="mb-1 text-[11px] text-zinc-400">{t1.name}</p>
              <div className="flex gap-4 text-xs">
                <span className="text-sky-400">
                  CT <span className="font-bold">{t1.map_stats_meta?.round_winrate_ct ?? "—"}%</span>
                </span>
                <span className="text-amber-400">
                  T <span className="font-bold">{t1.map_stats_meta?.round_winrate_t ?? "—"}%</span>
                </span>
              </div>
            </div>
            <div className="rounded-lg border border-white/5 bg-white/[0.02] p-3">
              <p className="mb-1 text-[11px] text-zinc-400">{t2.name}</p>
              <div className="flex gap-4 text-xs">
                <span className="text-sky-400">
                  CT <span className="font-bold">{t2.map_stats_meta?.round_winrate_ct ?? "—"}%</span>
                </span>
                <span className="text-amber-400">
                  T <span className="font-bold">{t2.map_stats_meta?.round_winrate_t ?? "—"}%</span>
                </span>
              </div>
            </div>
          </div>
        ...
        <section>
          ...
          <div className="mb-3 flex items-center gap-3">
            <div className="flex-1 rounded-lg border border-white/5 bg-white/[0.02] p-3 text-center">
              ...
              <p className="text-[10px] uppercase tracking-wide text-zinc-500">{t1.name}</p>
            </div>
            <span className="text-xs font-semibold text-zinc-500">vs</span>
            <div className="flex-1 rounded-lg border border-white/5 bg-white/[0.02] p-3 text-center">
              <p className="text-lg font-bold text-blue-400">{h2h.t2wins}</p>
              <p className="text-[10px] uppercase tracking-wide text-zinc-500">{t2.name}</p>
            </div>
          </div>
            ...
            <div className="space-y-1.5">
              {h2h.detail.slice(0, 10).map((d, i) => (
                ...
                >
                  ...
                  >
                    {d.winner === "T1" ? t1.name.slice(0, 3).toUpperCase() : d.winner === "T2" ? t2.name.slice(0, 3).toUpperCase() : "N"}
                  </span>
