...
export function BaseballMatchCard({ match, onOpen }: BaseballMatchCardProps) {
  ...
  return (
    <article className="group flex flex-col gap-3 rounded-xl border border-slate-800 bg-[#11161f] p-3.5 transition-colors hover:border-slate-600 sm:p-4">
      {/* Ligne méta */}
      <div className="flex flex-wrap items-center gap-2">
        <span
          className={`rounded border px-1.5 py-px text-[10px] font-bold uppercase tracking-wider ${LEAGUE_STYLES[game.league]}`}
        >
          {game.league === "MLB" ? "MLB 🇺🇸" : "KBO 🇰🇷"}
        </span>
        <span className="font-mono text-xs font-bold tabular-nums text-amber-300" title="Heure de Paris (CEST)">
