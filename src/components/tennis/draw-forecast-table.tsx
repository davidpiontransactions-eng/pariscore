...
/** Badge seed — fond neutre, texte theme. */
function SeedBadge({ seed }: { seed?: number }) {
  if (!seed) return null;
  return (
    <span className="inline-flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-muted font-mono text-[10px] font-bold text-foreground ring-1 ring-border">
      {seed}
    </span>
  );
}
...
function RankIcon({ rank }: { rank: number }) {
  if (rank === 0) return <Crown className="h-3.5 w-3.5 text-foreground" />;
  return (
...
/** Barre de progression — fond neutre, barre foreground. */
    ...
    <div className={cn("overflow-x-auto", className)}>
            ...
            {COLUMN_ROUNDS.map((r) => (
              <th
                ...
                className={cn(
                  ...
                  currentRound === r ? "text-foreground" : "text-muted-foreground",
                )}
              >
              ...
              >
                ...
                <td className="px-1 py-2.5">
                  <div className="flex items-center gap-2">
                    ...
                    <div className="flex flex-col">
                      <span
                        className={cn(
                          ...
                          isFav && "font-semibold text-foreground",
                          !isFav && "text-foreground/80",
                        )}
                      >
                        {row.name}
                      </span>
                    ...
                    {isFav && (
                      <Crown className="ml-auto h-3.5 w-3.5 text-muted-foreground/60" />
                    )}
                  </div>
                </td>
      ...
      <div className="mt-4 flex flex-wrap items-center gap-4 border-t border-border/40 pt-3 text-[10px] text-muted-foreground">
        ...
        <div className="ml-auto flex items-center gap-1">
          <Crown className="h-3 w-3 text-muted-foreground/60" />
          Favori titre
        </div>
      </div>
