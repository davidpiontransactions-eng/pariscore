...
| `0hf4` | **BSD coverage Phase 1+1.1+2** — TV channels + broadcasts (fetchers + caches + helper attach + on-demand route) | Phase 1 v12.65 `d10ab09` · Phase 1.1 cron pre-fetch v12.66 · Phase 2 ✅ route `GET /api/v1/broadcasts/match/:matchId` (wrap `fetchBSDBroadcastsByEvent` cache 6h, resolve matchId via `_bsdResolveEventId`) · Phase 3 UI chip/section reste ouverte |
