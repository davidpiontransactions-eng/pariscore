...
| `968x` | SEO/AEO Growth strategy — llms.txt + Structured Data + SSR scores + /about E-E-A-T | Phase 1 ✅ `llms.txt` + route GET `/llms.txt` Content-Type `text/markdown` cache 24h · Phase 4 ✅ `/about` E-E-A-T server-rendered standalone (4 piliers Experience / Expertise / Authoritativeness / Trustworthiness, méthodologie Poisson/edge/Power Score, sources canoniques 8 feeds, engagements conformité ARJEL/RGPD/18+, contact data) + ajout sitemap.xml priority 0.7 monthly + canonical/og:url. Phases 2 Structured Data JSON-LD + 3 SSR scores crawler ouvertes |
