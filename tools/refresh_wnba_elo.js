'use strict';
/**
 * refresh_nba_elo.js — Elo WNBA game-by-game (538 MOV-adjusted) + backtest Brier
 * Source : ESPN scoreboard ?dates=YYYYMMDD (gratuit). Output : data/wnba_elo.json
 *
 ...
 * Env  : WNBA_SEASON_START (YYYY-MM-DD, default 2026-05-01)
 *
...
const SB_PATH   = '/apis/site/v2/sports/basketball/wnba/scoreboard';
const OUTPUT    = path.join(__dirname, '..', 'data', 'wnba_elo.json');
const SEASON_START = process.env.WNBA_SEASON_START || '2026-05-01';
...
async function main() {
  console.log(`[WNBA-Elo] Season start ${SEASON_START} — fetching scoreboards…`);
  ...
  console.log(`[WNBA-Elo] ${games.length} matchs terminés — replay Elo + backtest…`);
  ...
  console.log(`[WNBA-Elo] OK — ${n} games · Brier ${payload.backtest.brier} (baseline ${payload.backtest.baseline_brier}) · acc ${payload.backtest.accuracy}% → ${OUTPUT}`);
}

if (require.main === module) main().catch(e => { console.error('[WNBA-Elo] FATAL', e); process.exit(1); });
