#!/usr/bin/env node
/**
 * bd 6jro Plan H — Sofascore initialFeaturedArticle editorial → modal Insights Avant-Match
 *
 * Ingest article éditorial Sofascore (title/excerpt/imageUrl/url/date) depuis dataset Apify
 * sofascore-scraper-pro → INSERT INTO api_cache (key='sofa_editorial_<normHome>_<normAway>')
 * source='sofascore_editorial' TTL 24h.
 *
 * Idempotent : skip si entrée non-expirée même source (sauf --force).
 * Football match uniquement (entries Tennis player profile skip — c'est Plan G).
 *
 * USAGE:
 *   node tools/import-sofascore-editorial.js [--dry-run] [--force]
 *
 * Datasets auto-detect racine projet : dataset_sofascore-scraper-pro_*.json
 */

const fs = require('fs');
const path = require('path');
const Database = require('better-sqlite3');

const ROOT = __dirname.replace(/\\tools$/, '').replace(/\/tools$/, '');
const DB_PATH = process.env.DATABASE_PATH || path.join(ROOT, 'pariscore.db');
const TTL_MS = 24 * 60 * 60 * 1000; // 24h
const SOURCE = 'sofascore_editorial';

const DRY_RUN = process.argv.includes('--dry-run');
const FORCE = process.argv.includes('--force');

function normKey(name) {
  if (!name) return '';
  return String(name).toLowerCase()
    .normalize('NFD').replace(/[̀-ͯ]/g, '')
    .replace(/[^a-z0-9 ]/g, ' ')
    .replace(/\s+/g, ' ').trim()
    .replace(/\s+/g, '_');
}

function findDatasets() {
  return fs.readdirSync(ROOT)
    .filter(f => /^dataset_sofascore-scraper-pro_.*\.json$/.test(f))
    .map(f => path.join(ROOT, f));
}

function loadEntries(file) {
  const raw = fs.readFileSync(file, 'utf8');
  const data = JSON.parse(raw);
  if (!Array.isArray(data)) return [];
  const out = [];
  for (const r of data) {
    if (!r || !r.url || !r.data) continue;
    if (!r.url.includes('/football/match/')) continue;
    const ev = r.data.event;
    if (!ev || !ev.homeTeam || !ev.awayTeam) continue;
    const article = Array.isArray(r.data.initialFeaturedArticle) ? r.data.initialFeaturedArticle[0] : null;
    if (!article || !article.title) continue;
    out.push({
      home_team: ev.homeTeam.name,
      away_team: ev.awayTeam.name,
      sofa_event_id: ev.id,
      article: {
        id: article.id,
        slug: article.slug,
        title: article.title,
        excerpt: article.excerpt,
        imageUrl: article.imageUrl,
        date: article.date,
        url: `https://www.sofascore.com/news/${article.slug}-${article.id}`,
        tags: Array.isArray(article.tags) ? article.tags.map(t => t.name).filter(Boolean).slice(0, 5) : [],
      },
    });
  }
  return out;
}

function run() {
  const datasets = findDatasets();
  if (datasets.length === 0) {
    console.log('  [Sofascore-editorial] Aucun dataset trouvé racine projet.');
    process.exit(0);
  }
  console.log(`  [Sofascore-editorial] ${datasets.length} dataset(s) found:`);
  datasets.forEach(d => console.log('    -', path.basename(d)));

  const db = DRY_RUN ? null : new Database(DB_PATH);
  try {
    if (db) db.pragma('journal_mode = WAL');
    const getStmt = db ? db.prepare('SELECT source, expires_at FROM api_cache WHERE key = ?') : null;
    const setStmt = db ? db.prepare('INSERT OR REPLACE INTO api_cache (key, data, source, created_at, expires_at) VALUES (?, ?, ?, ?, ?)') : null;

    let total = 0, written = 0, skipped = 0, dupes = new Set();
    const now = Date.now();
    const expires = now + TTL_MS;

    for (const file of datasets) {
      const entries = loadEntries(file);
      total += entries.length;
      for (const e of entries) {
        const h = normKey(e.home_team);
        const a = normKey(e.away_team);
        if (!h || !a) continue;
        const key = `sofa_editorial_${h}_${a}`;
        if (dupes.has(key)) continue;
        dupes.add(key);

        const data = {
          home_team: e.home_team,
          away_team: e.away_team,
          sofa_event_id: e.sofa_event_id,
          article: e.article,
        };

        if (db && !FORCE) {
          const existing = getStmt.get(key);
          if (existing && existing.expires_at > now && existing.source === SOURCE) {
            skipped++;
            continue;
          }
        }

        if (DRY_RUN) {
          console.log(`    [DRY] would write ${key} (${e.home_team} vs ${e.away_team}) — "${e.article.title.slice(0, 60)}..."`);
        } else {
          setStmt.run(key, JSON.stringify(data), SOURCE, now, expires);
        }
        written++;
      }
    }

    console.log(`  [Sofascore-editorial] total entries=${total} unique=${dupes.size} written=${written} skipped=${skipped}`);
    if (DRY_RUN) console.log('  [Sofascore-editorial] DRY RUN — aucun write effectué.');
  } finally {
    if (db) db.close();
  }
}

if (require.main === module) {
  try {
    run();
  } catch (e) {
    console.error('  [Sofascore-editorial] ERR:', e.message);
    process.exit(1);
  }
}

module.exports = { normKey };
