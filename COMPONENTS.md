...
| besoccer-elo-panel | besoccer-elo-panel.tsx | Comparatif Win probability 1X2 + blasons, style BeSoccer |
...
| handball-match-detail-dialog | handball-match-detail-dialog.tsx | Dialog détail 7 onglets : **Score** (matchs terminés seulement — scoreboard final + tableau Compte Match/1re MT/2e MT, ouverture par défaut sur 📆), Analyse (forme, cotes, verdict modèle), Stats, **Over & Buteurs** (échelle Over 59.5→52.5 seuil 55 %, pill pari ≥60 %, 1X2, winrate, splits buts L5/L10 dom/ext, PPG, 2 buteurs avec P(≥2/3/4/5) — via /api/handball/analysis), Bets (3 paris prédictifs + top GK/buteurs via /api/handball/players), **✨ IA** (analyse Gemini structurée 7 sections, générée 1× puis cache VPS 24 h — /api/handball/ai-analysis, markdown + disclaimer), **📊 Backtest** (perf walk-forward de la ligue du match, 8 marchés hit/ROI vs global — /api/handball/backtest-matrix, fetch lazy à la visite) |
...
| handball-backtest-matrix | handball-backtest-matrix.tsx | Matrice backtest 8 marchés × 16 championnats (source handball_match_history, cron lundi 04:40) — toggle Pleine période/30j, chips marchés ordre canonique, table ligues Hit/ROI avec ⚠️ n<10, ligne globale |
| handball-news | handball-news.tsx | Actus handball 5 sources (HandNews + Handball Planet RSS directs, L'Équipe + Eurosport + Handball365 via Google News RSS `site:`) — titres FR (traduction Gemini batch des titres EN, 1 appel/30 min, fallback original), badges source colorés, temps relatif fr, liens externes — /api/handball/news |
| handball-table-caption | handball-table-caption.tsx | Nom collé à chaque tableau handball (caption natif, lisible mobile : 11px bold uppercase, 12px desktop) — reste attaché au tableau dans les conteneurs overflow-x-auto (8 tableaux : backtest du jour, Simulé, CLV, matrice, Vitibet Top10, dialog Score/Compte, Forme L5-L10, Backtest par marché) |
| handball-live-commentary-panel | handball-live-commentary-panel.tsx | Suivi live commenté (xx78) — chronologie events FR (moteur pur `handball-live-commentary.ts`), jauge intensité 0-100, stats (arrêts/balles perdues/fautes/suspensions 2'), top buteurs, refresh 30 s sur /api/handball/live-detail ; sans clé API → « indisponible », jamais d'events fabricés |
