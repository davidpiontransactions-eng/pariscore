...
// Multi-window map stats caches (generated by tools/refresh_hltv_mapstats_multiwindow.js)
let _hltvMaps3mCache  = { ts: 0, byTeam: {}, mapRankings: {} };
let _hltvMaps6mCache  = { ts: 0, byTeam: {}, mapRankings: {} };
let _hltvMaps1yCache  = { ts: 0, byTeam: {}, mapRankings: {} };
...
function buildH2H(t1name, t2name, matches, n = 20) {
  ...
  // Détail par confrontation (maps jouées + scores orientés k1/k2), plus récent d'abord.
  const detail = h2h.map(m => {
    function _tn3(t) { return (typeof t === 'string' ? t : (t?.name || '')).toLowerCase(); }
    const winner = _tn3(m.winner); const normW = _normalizeTeamName(winner);
    let w = 'N';
    if (winner === k1 || normW === k1) w = 'T1';
    else if (winner === k2 || normW === k2) w = 'T2';
    const mt1 = _tn3(m.team1);
    const k1IsTeam1 = (mt1 === k1 || _normalizeTeamName(mt1) === k1);
    const maps = (m.maps || []).map(mp => {
      const s1 = Number(mp.team1_score);
      const s2 = Number(mp.team2_score);
      const v1 = Number.isFinite(s1) ? s1 : null;
      const v2 = Number.isFinite(s2) ? s2 : null;
      return {
        name: mp.name || null,
        k1_score: k1IsTeam1 ? v1 : v2,
        k2_score: k1IsTeam1 ? v2 : v1,
      };
    });
    return { date: m.date || null, winner: w, maps };
  });

  return { t1wins, t2wins, results, detail, n: h2h.length, last_date: h2h[0]?.date || null };
}
...
// ─── Map World Rankings from HLTV local JSON (top-30, real data) ─────────────
// Primary: data/hltv_team_mapstats.json (all-time, generated by refresh script)
// Also loads HLTV rankings for ELO-equiv rank points.
// Secondary fallback: BSD ELO-sorted teams (poor map coverage, tier-3 heavy).
let _bsdMapRankingsCache = { ts: 0, rankings: null };
const BSD_MAP_RANKINGS_TTL = 6 * 60 * 60 * 1000; // 6h
...
async function computeBSDMapRankings(topN = 30) {
  ...
  try {
    ...
    if (Object.keys(mapData).length < 5 && _bsdApiKeyRef) {
      ...
      // Use unique IDs to avoid duplicates from multi-key byName index
      const seen = new Set();
      const sorted = Object.values(bsdTeams)
        .filter(t => { if (!t.id || !t.elo_rating || seen.has(t.id)) return false; seen.add(t.id); return true; })
      ...
      for (let i = 0; i < sorted.length; i += 5) {
        ...
        for (let j = 0; j < batch.length; j++) {
          ...
          if (!detail) continue;
          const norm = _normalizeTeamName((detail.name || '').toLowerCase()) || (detail.name||'').toLowerCase();
          const mLookup = detail.map_stats ? _bsdMapStatsToLookup(detail.map_stats) : {};
          if (!mapData[norm]) mapData[norm] = { ...mLookup, _rank: 99, _pts: 0, _name: detail.name || norm };
        }
      }
    }
    ...
    const rankings = {};
    for (const map of ACTIVE_MAPS) {
      const mapKey = map; // "Mirage", "Inferno" etc. — HLTV JSON uses capitalized keys
      const entries = Object.entries(mapData)
        .filter(([, d]) => d[mapKey] != null)
        .map(([, d]) => ({ name: d._name, rank_pts: d._pts, hltv_rank: d._rank, wr: d[mapKey] }))
        .sort((a, b) => b.wr - a.wr)
        .map((e, i) => ({ ...e, rank: i + 1 }));
      rankings[map] = entries;
    }
    _bsdMapRankingsCache = { ts: Date.now(), rankings };
    const teamCount = Object.keys(mapData).length;
    console.log(`[CS2/MapRank] Rankings built: ${teamCount} teams, source=hltv-json+bsd`);
    return rankings;
  } catch (e) {
    console.warn('[CS2/MapRank]', e.message);
    return _bsdMapRankingsCache.rankings;
  }
}

// Get a team's rank on a specific map from world rankings
async function getTeamMapRankBSD(teamName, mapName) {
  ...
  const mkey = mapName.charAt(0).toUpperCase() + mapName.slice(1); // "Mirage"
  const list = rankings[mkey] || rankings[mapName] || [];
  return list.find(t => {
    const n = _normalizeTeamName((t.name||'').toLowerCase()) || (t.name||'').toLowerCase();
    return n === key;
  }) || null;
}
...
function _loadHltvWindowFile(filePath, cache) {
  ...
  try {
    ...
    for (const t of (raw.teams || [])) {
      if (!t.name) continue;
      byTeam[t.name.toLowerCase()] = t.maps || {};
      const norm = _normalizeTeamName(t.name);
      if (norm && norm !== t.name.toLowerCase()) byTeam[norm] = t.maps || {};
    }
    const mapRankings = raw.map_rankings || {};
    cache.ts = Date.now(); cache.byTeam = byTeam; cache.mapRankings = mapRankings;
    console.log(`[CS2/MapStats] ${raw.window || '?'} window: ${Object.keys(byTeam).length} teams`);
  } catch (e) { console.warn('[CS2/MapStats-MW]', e.message); }
  return cache;
}

function loadHltvMapStatsMulti() {
  _loadHltvWindowFile(HLTV_MAPSTATS_3M, _hltvMaps3mCache);
  _loadHltvWindowFile(HLTV_MAPSTATS_6M, _hltvMaps6mCache);
  _loadHltvWindowFile(HLTV_MAPSTATS_1Y, _hltvMaps1yCache);
}

// ─── Map Trend: compare 3m vs 6m vs 1y winrate for a team on a given map ─────
// Returns { wr_3m, wr_6m, wr_1y, trend, map_rank_3m, map_rank_6m }
// trend: 'rising' (3m>6m+5), 'declining' (3m<6m-5), 'stable'
function computeMapTrend(teamName, mapName) {
  loadHltvMapStatsMulti();
  const key  = _normalizeTeamName(teamName) || (teamName || '').toLowerCase();
  const mkey = (mapName || '').charAt(0).toUpperCase() + (mapName || '').slice(1).toLowerCase();
  const wr3m = _hltvMaps3mCache.byTeam[key]?.[mkey] ?? null;
  const wr6m = _hltvMaps6mCache.byTeam[key]?.[mkey] ?? null;
  const wr1y = _hltvMaps1yCache.byTeam[key]?.[mkey] ?? null;
  const trend = (wr3m != null && wr6m != null)
    ? (wr3m - wr6m >=  5 ? 'rising'
     : wr3m - wr6m <= -5 ? 'declining'
     : 'stable')
    : null;
  // Map rank from pre-computed rankings
  const rank3m = (_hltvMaps3mCache.mapRankings[mkey] || []).find(t => _normalizeTeamName(t.name) === key)?.rank ?? null;
  const rank6m = (_hltvMaps6mCache.mapRankings[mkey] || []).find(t => _normalizeTeamName(t.name) === key)?.rank ?? null;
  if (wr3m == null && wr6m == null && wr1y == null) return null;
  return { wr_3m: wr3m, wr_6m: wr6m, wr_1y: wr1y, trend, map_rank_3m: rank3m, map_rank_6m: rank6m };
}

// Compute map trends for all active maps for a team
function computeAllMapTrends(teamName) {
  const out = {};
  let hasData = false;
  for (const map of ACTIVE_MAPS) {
    const t = computeMapTrend(teamName, map);
    if (t) { out[map] = t; hasData = true; }
  }
