...
// Source 5 : beSOCCER via CDN resfu.com (mapping pré-backfillé par Camoufox)
// ────────────────────────────────────────────────────────────────────────────
// beSOCCER (fr.besoccer.com) est protégé par un WAF Fastly (406 + JS challenge) →
// inaccessible en fetch natif. Mais son CDN logos public cdn.resfu.com est OUVERT :
//   https://cdn.resfu.com/img_data/escudos/medium/{resfu_id}.jpg?size=120x&lossy=1
// Le mapping {name_norm → resfu_id} est peuplé une fois pour toutes par
// scripts/besoccer-backfill-ids.py (Camoufox stealth, ~3min/équipe, collecte les
// adversaires en bonus). Cette source ne fait QUE lire le JSON — zéro runtime Camoufox,
// zéro réseau (le CDN resfu est servi par le frontend directement via logo_url).
//
// Faux positifs connus : le backfill collecte parfois des joueurs (initiales D./I./M.)
// à côté de shields d'équipe. On filtre les noms commençant par une initiale + point.
const _RESFU_ID_PATH = path.join(__dirname, '..', 'data', 'resfu-ids.json');
let _resfuMapping = null; // cache lazy (1 load par vie du process)
function loadResfuMapping() {
  if (_resfuMapping !== null) return _resfuMapping;
  try {
    const txt = fs.readFileSync(_RESFU_ID_PATH, 'utf8');
    _resfuMapping = JSON.parse(txt);
  } catch (_) {
    _resfuMapping = {}; // fichier absent ou illisible → source désactivée silencieusement
  }
  return _resfuMapping;
}
const _PLAYER_NAME_RE = /^[A-Z]\.\s/; // "D. Dumfries", "I. Konate" (faux positifs du backfill)

function sourceBeSoccer(name) {
  const mapping = loadResfuMapping();
  if (!mapping || Object.keys(mapping).length === 0) return null;
  if (_PLAYER_NAME_RE.test(name)) return null; // filtre faux positifs joueurs
  const norm = normalizeTeamName(name);
  if (!norm) return null;
  let entry = mapping[norm];
  if (!entry && norm.length >= 3) {
    // Fuzzy : préfixe ou contenance
    const keys = Object.keys(mapping);
    entry = keys.find(k => k.startsWith(norm) || norm.startsWith(k))
      ? mapping[keys.find(k => k.startsWith(norm) || norm.startsWith(k))]
      : null;
  }
  if (!entry || !entry.resfu_id) return null;
  return {
    bsd_id: externalId(name),
    name,
    short_name: '',
    country: '',
    logo_url: `https://cdn.resfu.com/img_data/escudos/medium/${entry.resfu_id}.jpg?size=120x&lossy=1`,
    source: 'besoccer',
  };
}
...
async function resolveTeamLogo(name, opts = {}) {
  ...
  // 4. beSOCCER (CDN resfu — lecture mapping JSON pré-backfillé, zéro réseau)
  //    Désactivé silencieusement si data/resfu-ids.json absent. Sync (pas await).
  try {
    const r = sourceBeSoccer(name);
    if (r) return r;
  } catch (e) { if (opts.verbose) console.warn(`  [besoccer] ${e.message}`); }

  // 5. Scraping BSD HTML (seulement si matchId fourni)
  if (opts.matchId) {
    try {
